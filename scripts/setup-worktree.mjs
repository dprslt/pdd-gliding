        ...
        for (const rel of expand(pattern)) {
            ...
            if (st) {
                if (st.isSymbolicLink()) {
                    ...
                    log(
                        `skip ${rel} (a real file/dir already exists in this worktree)`,
                    );
                    continue;
                }
            }
...
const seedNodeModules = () => {
    ...
    if (process.platform === 'darwin')
        cow = ['cp', ['-cR', src, dest]]; // APFS clonefile
    else if (process.platform === 'linux')
        cow = ['cp', ['-R', '--reflink=auto', src, dest]];
    else if (process.platform === 'win32')
        cow = [
            'robocopy',
            [src, dest, '/E', '/NJH', '/NJS', '/NDL', '/NP', '/R:0'],
        ];
    ...
    log(
        `no copy-on-write path on ${process.platform} — leaving node_modules for "yarn install"`,
    );
};
